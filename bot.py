...
@bot.message_handler(func=lambda message: db_users.get_current_state(message.from_user.id) == config.STATES['state_1'])
def get_user_name(message):
    checking_name = validators.check_name(message.text)
    if checking_name:
        bot.send_message(message.from_user.id, f'{checking_name}. Попробуйте снова.')
        command_start(message)
    else:
        db_users.cur.execute(f"UPDATE user_questionnaire SET name = '{message.text}' WHERE id = {message.from_user.id}")
        db_users.con.commit()
        bot.send_message(message.from_user.id, 'Ваш возраст?:', reply_markup=keyboard_back)
...
@bot.message_handler(func=lambda message: db_users.get_current_state(message.from_user.id) == config.STATES['state_3'])
def get_user_sex(message):
    ...
    if message.text == 'Назад':
        ...
        db_users.cur.execute(f"UPDATE user_questionnaire SET sex = '{message.text}' WHERE id = {message.from_user.id}")
        db_users.con.commit()
        bot.send_message(message.from_user.id, 'Главное меню:', reply_markup=keyboard_menu)
        db_users.set_new_state(message.from_user.id, config.STATES['state_4'])


@bot.message_handler(func=lambda message: db_users.get_current_state(message.from_user.id) == config.STATES['state_4'])
def menu(message):
    if message.text == 'Инфа про меня':
        db_users.cur.execute(f'SELECT name, age, sex FROM user_questionnaire WHERE id = {message.from_user.id}')
        info = db_users.cur.fetchone()
        bot.send_message(message.from_user.id, f'Ваше имя: {info[0]}\nВаш возраст: {info[1]}\nВаш пол: {info[2]}',
...
@bot.message_handler(func=lambda message: db_users.get_current_state(message.from_user.id) == config.STATES['state_5'])
def back_info(message):
    if message.text == 'Назад':
        come_back_to_sex(message)


@bot.message_handler(func=lambda message: db_users.get_current_state(message.from_user.id) == config.STATES['state_6'])
def settings(message):
    if message.text == 'Сменить имя':
        bot.send_message(message.from_user.id, 'Введите новое имя:', reply_markup=keyboard_back)
        db_users.set_new_state(message.from_user.id, config.STATES['state_7'])
    elif message.text == 'Сменить возраст':
        bot.send_message(message.from_user.id, 'Введите новый возраст:', reply_markup=keyboard_back)
        db_users.set_new_state(message.from_user.id, config.STATES['state_8'])
    elif message.text == 'Сменить пол':
        bot.send_message(message.from_user.id, 'Введите новый пол:', reply_markup=keyboard_sex)
        db_users.set_new_state(message.from_user.id, config.STATES['state_9'])
    elif message.text == 'Назад':
        come_back_to_sex(message)


@bot.message_handler(func=lambda message: db_users.get_current_state(message.from_user.id) == config.STATES['state_7'])
...
@bot.message_handler(func=lambda message: db_users.get_current_state(message.from_user.id) == config.STATES['state_9'])
...
def come_back_to_name(message):
    db_users.set_new_state(message.from_user.id, config.STATES['state_1'])
    db_users.cur.execute(f'SELECT name FROM user_questionnaire WHERE id = {message.from_user.id}')
...
def come_back_to_age(message):
    db_users.set_new_state(message.from_user.id, config.STATES['state_2'])
    db_users.cur.execute(f'SELECT age FROM user_questionnaire WHERE id = {message.from_user.id}')
...
def come_back_to_sex(message):
    db_users.set_new_state(message.from_user.id, config.STATES['state_3'])
    db_users.cur.execute(f'SELECT sex FROM user_questionnaire WHERE id = {message.from_user.id}')
